...
# Center the image by creating a 3-column layout and putting the image in the middle column
left_col, mid_col, right_col = st.columns([1, 2, 1])
with mid_col:
    # Use width parameter to make image 70% of its original size (30% smaller)
    st.image("header.png", use_container_width=True)  # Image fills the middle column width
...
def process_uploaded_file(uploaded_file, logger):
    ...
    try:
        ...
        df_transformed, platform_dfs, platform_counts = find_platform_tables_and_transform(df_full_sheet, logger, error_collector)

        if df_transformed.empty:
            logger.warning("Transformation resulted in an empty dataset.")
        else:
            logger.info(f"Successfully transformed data. Generated {len(df_transformed)} total combinations (before deduplication for unique count). Unique combinations: {len(df_transformed.drop_duplicates())}")

        error_summary = error_collector.get_summary()
        if error_summary:
            st.warning("The following issues were detected during processing:")
